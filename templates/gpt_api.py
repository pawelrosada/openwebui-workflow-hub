#!/usr/bin/env python3
"""
OpenAI GPT-4o API - Individual Model Server
Dedicated FastAPI server for OpenAI GPT-4o model.

Features:
- Dedicated server for gpt model on port 8002
- Flow ID routing for different workflows
- Direct LangFlow integration
- Docker-compatible setup

Usage:
    GPT_API_KEY="your-key-here" python gpt_api.py
...
app = fastapi.FastAPI(
    title="OpenAI GPT-4o LangFlow API",
    description="Dedicated API for OpenAI GPT-4o through LangFlow",
    version="1.0.0",
)
...
MODEL_CONFIG = {
    "model": "gpt-4o",
    "api_key": os.getenv("GPT_API_KEY", "<YOUR_GPT_API_KEY>"),
...
async def root():
    ...
    return {
        "name": "OpenAI GPT-4o LangFlow API",
        "version": "1.0.0",
        "model": MODEL_CONFIG["model"],
        "port": 8002,
        "configured": MODEL_CONFIG["api_key"] != "<YOUR_GPT_API_KEY>",
    }
...
async def invoke_flow(flow_id: str, request: ChatRequest):
    """
    Invoke a LangFlow workflow with OpenAI GPT-4o
    ...
    if MODEL_CONFIG["api_key"] == "<YOUR_GPT_API_KEY>":
        raise fastapi.HTTPException(
            status_code=400,
            detail="API key not configured. Set GPT_API_KEY environment variable.",
        )
    ...
    try:
        ...
        payload = {
            ...
            "tweaks": {
                ...
                "gpt_api_key": MODEL_CONFIG["api_key"],
                "model": MODEL_CONFIG["model"],
        ...
        logger.info(f"Invoking flow {flow_id} with gpt")
...
if __name__ == "__main__":
    ...
    print(f"🚀 Starting OpenAI GPT-4o API on port 8002")
    print(f"🤖 Model: {MODEL_CONFIG['model']}")
    print(f"🔗 LangFlow URL: {LANGFLOW_BASE_URL}")
    print(f"📖 API docs: http://localhost:8002/docs")
    print(f"🔑 API key configured: {MODEL_CONFIG['api_key'] != '<YOUR_GPT_API_KEY>'}")

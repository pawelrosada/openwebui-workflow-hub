#!/usr/bin/env python3
"""
Anthropic Claude 3.5 Sonnet API - Individual Model Server
Dedicated FastAPI server for Anthropic Claude 3.5 Sonnet model.

Features:
- Dedicated server for claude model on port 8003
- Flow ID routing for different workflows
- Direct LangFlow integration
- Docker-compatible setup

Usage:
    CLAUDE_API_KEY="your-key-here" python claude_api.py
...
app = fastapi.FastAPI(
    title="Anthropic Claude 3.5 Sonnet LangFlow API",
    description="Dedicated API for Anthropic Claude 3.5 Sonnet through LangFlow",
    version="1.0.0",
)
...
async def root():
    ...
    return {
        "name": "Anthropic Claude 3.5 Sonnet LangFlow API",
        "version": "1.0.0",
        "model": MODEL_CONFIG["model"],
        "port": 8003,
        "configured": MODEL_CONFIG["api_key"] != "<YOUR_CLAUDE_API_KEY>",
    }
...
async def invoke_flow(flow_id: str, request: ChatRequest):
    """
    Invoke a LangFlow workflow with Anthropic Claude 3.5 Sonnet
    ...
    if MODEL_CONFIG["api_key"] == "<YOUR_CLAUDE_API_KEY>":
        raise fastapi.HTTPException(
            status_code=400,
            detail="API key not configured. Set CLAUDE_API_KEY environment variable.",
        )
    ...
    try:
        ...
        payload = {
            ...
            "tweaks": {
                ...
                "claude_api_key": MODEL_CONFIG["api_key"],
                "model": MODEL_CONFIG["model"],
        ...
        logger.info(f"Invoking flow {flow_id} with claude")
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            return ChatResponse(response=text, flow_id=flow_id, status="success")
...
if __name__ == "__main__":
    ...
    print(f"🚀 Starting Anthropic Claude 3.5 Sonnet API on port 8003")
    print(f"🤖 Model: {MODEL_CONFIG['model']}")
    print(f"🔗 LangFlow URL: {LANGFLOW_BASE_URL}")
    print(f"📖 API docs: http://localhost:8003/docs")
    print(
        f"🔑 API key configured: {MODEL_CONFIG['api_key'] != '<YOUR_CLAUDE_API_KEY>'}"
    )

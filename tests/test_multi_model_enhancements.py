...
def test_enhanced_pipeline():
    ...
    try:
        ...
        test_cases = [
            ...
            (
                "@agent How do I learn Python?",
                ("claude", "How do I learn Python?"),
        ...
        for input_text, expected in test_cases:
            ...
            if result == expected or (
                result[0] in ["gemini", "gpt", "claude"] and result[1] == expected[1]
            ):
                print(f"   ✅ Parsing test passed: '{input_text[:30]}...'")
            else:
                print(
                    f"   ❌ Parsing test failed: '{input_text[:30]}...' -> {result} (expected {expected})"
                )
        ...
        for content, expected_model in content_tests:
            ...
            if selected == expected_model:
                ...
                print(
                    f"   ⚠️  Content routing: '{content[:30]}...' -> {selected} (expected {expected_model})"
                )
...
def test_api_templates():
    ...
    generators = ["generate_multi_scripts.py", "generate_universal_workflows.py"]
...
def test_workflow_templates():
    ...
    templates_dir = (
        Path(__file__).parent.parent / "templates" / "examples" / "langflow-workflows"
    )
    ...
    workflows = ["universal-multi-model-chat.json", "agentic-multi-model-router.json"]
        ...
        if workflow_path.exists():
            try:
                ...
                if all(key in data for key in required_keys):
                    ...
                    print(
                        f"   ✅ {workflow_file}: {len(nodes)} nodes, {len(edges)} edges"
                    )
                    print(f"      📝 {data['name']}")
                else:
...
def test_generated_multi_scripts():
    ...
    expected_files = ["gemini_api.py", "gpt_api.py", "claude_api.py", "orchestrator.py"]

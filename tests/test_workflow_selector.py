...
class TestWorkflowSelectorPipeline:
    ...
    def test_parse_workflow_directive_list_workflows(self):
        ...
        action, workflow_id, message = self.pipeline.parse_workflow_directive(
            "@workflows", "session1"
        )
    ...
    def test_parse_workflow_directive_set_workflow(self):
        ...
        action, workflow_id, message = self.pipeline.parse_workflow_directive(
            "@set-workflow:basic-chat", "session1"
        )
    ...
    def test_parse_workflow_directive_use_workflow_name(self):
        ...
        action, workflow_id, message = self.pipeline.parse_workflow_directive(
            "@workflow:basic-chat Hello there", "session1"
        )
    ...
    def test_parse_workflow_directive_use_workflow_id(self):
        ...
        action, workflow_id, message = self.pipeline.parse_workflow_directive(
            "@flow:workflow-1-id How are you?", "session1"
        )
    ...
    def test_parse_workflow_directive_use_current(self):
        ...
        action, workflow_id, message = self.pipeline.parse_workflow_directive(
            "Just a regular message", "session1"
        )
    ...
    def test_resolve_workflow_id(self):
        ...
        # Direct ID usage
        result = self.pipeline.resolve_workflow_id(
            "use_workflow_id", "direct-workflow-id"
        )
    ...
    def test_handle_set_workflow_sync(self):
        ...
        self.pipeline.cache_timestamp = (
            9999999999  # Far future to prevent cache refresh
        )
    ...
    def test_call_langflow_with_workflow_success(self, mock_client):
        ...
        mock_response.json.return_value = {
            "outputs": [
                {
                    "outputs": [
                        {"results": {"message": {"text": "Hello from workflow!"}}}
                    ]
                }
            ]
        }
        ...
        result = list(
            self.pipeline.call_langflow_with_workflow(
                "use_workflow_id",
                "workflow-1-id",
                "test message",
                "session1",
    ...
    def test_edge_cases(self):
        ...
        action, workflow_id, message = self.pipeline.parse_workflow_directive(
            "@workflow:BASIC-CHAT test", "session1"
        )
        assert action == "use_workflow_name"
        assert workflow_id == "basic-chat"
        assert message == "test"
...
def test_pipeline_integration():
    ...
    test_body = {"user": {"id": "test_user"}, "chat_id": "test_chat"}

...
class Pipeline:
    class Valves(BaseModel):
        ...
        DEFAULT_WORKFLOW_ID: str = Field(
            default="3ec49b62-4a8e-4cb9-9913-a51086ca7471",
            description="Fallback workflow ID",
        )

        # Authentication (if needed)
        LANGFLOW_API_KEY: str = Field(
            default="", description="Langflow API key (if authentication enabled)"
        )

        # Workflow Discovery
        ENABLE_WORKFLOW_DISCOVERY: bool = Field(
            default=True, description="Enable automatic workflow discovery"
        )
        WORKFLOW_CACHE_TTL: int = Field(
            default=300, description="Workflow cache TTL in seconds"
        )

        # Session Management
        ENABLE_SESSION_MEMORY: bool = Field(
            default=True, description="Remember workflow selection per user session"
        )

        # Performance
        RATE_LIMIT: int = Field(default=5, description="Requests per second limit")
        CONNECTION_TIMEOUT: int = Field(
            default=30, description="HTTP timeout in seconds"
        )
    ...
    async def discover_workflows(self) -> Dict[str, Any]:
        ...
        try:
            ...
            ) < self.valves.WORKFLOW_CACHE_TTL and self.workflow_cache:
                ...
                for endpoint in endpoints_to_try:
                    try:
                        ...
                        if response.status_code == 200:
                            data = response.json()
                            logger.info(
                                f"Successfully discovered workflows from {endpoint}"
                            )
                            ...
                            if isinstance(data, list):
                                for workflow in data:
                                    if isinstance(workflow, dict):
                                        flow_id = workflow.get(
                                            "id", workflow.get("flow_id", "")
                                        )
                                        name = workflow.get(
                                            "name",
                                            workflow.get(
                                                "title", f"Workflow {flow_id[:8]}"
                                            ),
                                        )
                                        if flow_id:
                                            workflows[
                        ...
                        logger.debug(
                            f"Failed to discover workflows from {endpoint}: {e}"
                        )
                        continue
    ...
    def parse_workflow_directive(
        ...
        set_match = re.match(
            r"@set-workflow:(\S+)\s*$", user_message.strip(), re.IGNORECASE
        )
        if set_match:
        ...
        workflow_match = re.match(
            r"@workflow:(\S+)\s+(.*)", user_message, re.IGNORECASE
        )
        if workflow_match:
    ...
    def pipe(
        ...
        action, workflow_identifier, cleaned_message = self.parse_workflow_directive(
        ...
        return "".join(
            [
                chunk
                for chunk in self.call_langflow_with_workflow(
    ...
    def resolve_workflow_id(self, action: str, workflow_identifier: str) -> str:
        ...
        if action == "use_workflow_id" or action == "use_current":
            ...
            if workflow_identifier in self.workflow_cache:
                ...
                logger.warning(
                    f"Workflow name '{workflow_identifier}' not found, using default"
                )
                return self.valves.DEFAULT_WORKFLOW_ID
        else:
            return self.valves.DEFAULT_WORKFLOW_ID

    def call_langflow_with_workflow(
        self,
        action: str,
        ...
        logger.info(
            f"Calling workflow '{workflow_name}' (ID: {workflow_id[:8]}...) for session {session_id}"
        )
        ...
        try:
            ...
            logger.error(
                f"HTTP error {e.response.status_code} calling Langflow workflow {workflow_id}: {e}"
            )
            ...
            logger.error(
                f"Unexpected error calling Langflow workflow {workflow_id}: {e}"
            )

...
class Pipeline:
    class Valves(BaseModel):
        ...
        ENABLE_MULTI_MODEL: bool = Field(
            default=True, description="Enable multi-model support"
        )
        DEFAULT_MODEL: str = Field(
            default="gpt", description="Default model (gemini, gpt, claude)"
        )

        # Model-specific workflow IDs (optional - uses WORKFLOW_ID if not set)
        GEMINI_WORKFLOW_ID: str = Field(
            default="", description="Specific workflow for Gemini"
        )
        GPT_WORKFLOW_ID: str = Field(
            default="", description="Specific workflow for GPT"
        )
        CLAUDE_WORKFLOW_ID: str = Field(
            default="", description="Specific workflow for Claude"
        )

        # Universal workflow support
        UNIVERSAL_WORKFLOW_ID: str = Field(
            default="", description="Universal workflow that handles all models"
        )
        ...
        ANTHROPIC_API_KEY: str = Field(
            default="", description="Anthropic API key for tweaks"
        )

        # Rate limiting and performance
        RATE_LIMIT: int = Field(default=5, description="Requests per second limit")
        ENABLE_AGENTIC_ROUTING: bool = Field(
            default=True, description="Enable smart model routing"
        )
    ...
    def parse_model_directive(self, user_message: str) -> tuple[str, str]:
        ...
        if model_match:
            ...
            if requested_model in self.models:
                ...
                logger.warning(
                    f"Unknown model requested: {requested_model}, using default"
                )
        ...
        if agent_match and self.valves.ENABLE_AGENTIC_ROUTING:
            ...
            logger.info(
                f"Agent routing: '{cleaned_message[:50]}...' -> {selected_model}"
            )
    ...
    def get_model_tweaks(self, model: str, temperature: float = 0.7) -> Dict[str, Any]:
        ...
        base_tweaks = {"temperature": temperature, "model": self.models[model]["model"]}
    ...
    def pipe(
        ...
        return "".join(
            [
                chunk
                for chunk in self.call_langflow(
                    cleaned_message, selected_model, dt_start
                )
            ]
        )
